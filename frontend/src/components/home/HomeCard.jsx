  ...
  return (
    <Link href={`#`} key={data?.id}>
      <div className="lg:max-w-48 w-full shadow rounded-md bg-white">
        ...
        <div className="p-1 lg:p-3 mt-1">
          <h3 className="font-medium text-sm lg:text-lg leading-6">
            {textSlicer(`${data?.title}`, 30)}
          </h3>
          <div className="flex items-center gap-1 pt-2">
            <p className="pt-2 text-base lg:text-lg font-semibold">
              ৳ {data?.price}
            </p>
            <p className="pt-3 text-xs lg:text-sm text-gray-400 font-medium line-through">
              ৳ {data?.mainPrice}
            </p>
          </div>
        </div>
        <div className="px-1 lg:px-3 pb-3 flex justify-between">
